...
import { Loading, SectionHeader, CategoryPill } from '@components/ui'
import { FeaturedCard, GenreTile, PopularRow, MovieCard } from '@components/movie'
...
export default function HomeScreen() {
  ...
  const { newMovies, singleMovies, isLoading, refetch, isRefetching } = useHomeMovies()
  const { items: watchlistItems } = useWatchlist()
  const theme = useTheme()

  const contentStyle = useMemo(
    () => ({ paddingTop: insets.top, paddingBottom: 96 }),
    [insets.top]
  )
    ...
    >
      ...
      <View style={s.header}>
        ...
        <View style={s.flex1}>
          ...
          <Text style={[s.subGreeting, { color: theme.text2 }]}>Chúc xem phim vui vẻ</Text>
        </View>
        <Pressable onPress={() => router.push('/(tabs)/search')} style={[s.iconBtn, { backgroundColor: theme.surface }]}>
          <MaterialCommunityIcons name="magnify" size={18} color={theme.text2} />
        </Pressable>
        <Pressable onPress={() => router.push('/(tabs)/library')} style={[s.iconBtn, { backgroundColor: theme.surface }]}>
          <MaterialCommunityIcons name="bookmark-outline" size={18} color={theme.text2} />
        </Pressable>
      </View>
      ...
      >
        {CATEGORIES.map(c => (
          <CategoryPill key={c} label={c} active={cat === c} onPress={() => setCat(c)} />
        ))}
      </ScrollView>
          ...
          >
            {continueWatching.map((m, i) => {
              ...
              return (
                ...
                >
                  <View style={[s.continuePoster, { backgroundColor: theme.surface }]}>
                    <Image
                    ...
                    <View style={[s.progressBadge, { backgroundColor: 'rgba(0,0,0,0.6)' }]}>
                      <Text style={s.progressText}>{Math.round(progress * 100)}%</Text>
                    </View>
                  </View>
                  <Text style={[s.continueTitle, { color: theme.text }]} numberOfLines={1}>{m.name}</Text>
                  <Text style={[s.continueMeta, { color: theme.text2 }]}>{m.language}</Text>
                </Pressable>
              )
            })}
          </ScrollView>
      ...
      {trending.length > 0 && (
        <View style={s.section}>
          ...
          >
            {trending.map(m => <MovieCard key={m.slug} movie={m} />)}
          </ScrollView>
        </View>
      )}
            ...
            <View key={`genre-row-${ri}`} style={s.genreRowInner}>
              {row.map(g => (
                <View key={g.id} style={s.flex1}>
                  <GenreTile genre={g} onPress={() => router.push({ pathname: '/genre/[slug]', params: { slug: g.id } })} />
                </View>
              ))}
            </View>
          ))}
      ...
      {recommended.length > 0 && (
        <View style={s.section}>
          ...
          <ScrollView
            ...
            contentContainerStyle={s.movieRow}
          >
            {recommended.map(m => <MovieCard key={m.slug} movie={m} />)}
          </ScrollView>
        </View>
      )}
      ...
      {popular.length > 0 && (
        <View style={[s.section, s.padH]}>
          ...
          {popular.map((m, i) => <PopularRow key={m.slug} movie={m} index={i + 1} />)}
        </View>
      )}
    </ScrollView>
  )
